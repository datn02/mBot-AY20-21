...
void colourAlgorithm() {
  ...
  switch (colourCheck) {
    ...
    case 2: {
      ...
      // decides to do 2 left/right turns depending on IR reading from each wall
      if (voltage_left < 3.5) {
        turn_right();
        turn_right();
        break;
...
void adjustWallProximity() {
  ...
  if (voltage_left - 3.5 < -0.00001) {
    Left_Motor.run(-1 * (left_move_speed + gain_left * (3.5 - voltage_left)));
    Right_Motor.run(move_speed - gain_left * (3.5 - voltage_left));
    while (true) {
  ...
  else if (voltage_right - 3.60 < -0.00001) {
    Left_Motor.run(-1 * (left_move_speed - gain_right * (3.60 - voltage_right)));
    Right_Motor.run(move_speed + gain_right * (3.60 - voltage_right));
    while (true) {
      if (lineFinder.readSensors() == S1_IN_S2_IN) {
        Left_Motor.stop();
        Right_Motor.stop();
...
void loop() {
  ...
  //The mBot constantly checks and adjuts if it is too near to either wall
  else {
    Left_Motor.run(-1 * left_move_speed);
    Right_Motor.run(move_speed);
    adjustWallProximity();
